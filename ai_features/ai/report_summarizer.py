...
class ReportSummarizer:
    ...
    def _extract_section(self, text: str, section_type: str) -> str:
        """Extract content related to a specific section type using a sliding window approach"""
        sentences = sent_tokenize(text)
        section_content = []
        window_size = 3  # Number of sentences to consider together
        threshold = 0.6  # Base threshold for classification
        
        # Process sentences in windows to capture context
        for i in range(0, len(sentences), window_size):
            window = sentences[i:i + window_size]
            window_text = " ".join(window)
            
            # Classify the entire window
            ...
            adjusted_threshold = threshold - (1 - result['scores'][0]) * 0.2
            
            if result['labels'][0] == f"{section_type} related" and result['scores'][0] > adjusted_threshold:
                section_content.extend(window)
            
            # Add individual sentence checks for important content
            for sentence in window:
                ...
                # Allow slightly lower threshold for individual sentences
                if result['labels'][0] == f"{section_type} related" and result['scores'][0] > threshold - 0.1:
                    section_content.append(sentence)
        
        # Remove duplicates while preserving order
        seen = set()
        section_content = [s for s in section_content if not (s in seen or seen.add(s))]
